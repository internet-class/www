...
var plugin = function (config) {
  return function (files, metalsmith, done) {
    try {
      assert(config.credentialsFile);
      assert(fs.existsSync(path.join(metalsmith.source(), config.credentialsFile)));
      assert(config.tokenFile);
      if (config.test) {
        assert(config.testPlaylist);
      }
    } catch (err) {
      return done(new Error("incorrect youtube_credentials configuration"));
    }
    retrieve({
