  ...
  async.series([
    function (callback) {
      ...
      if (transcodeInfo.creditsFile) {
        ...
        if (config.veryVerbose) {
          console.log(combineCreditsCommandLine);
        }
        child_process.exec(combineCreditsCommandLine + ' > output.txt 2>&1', { cwd: workingDir },
          function (err) {
            try {
              assert(!err);
              ...
              throw new Error("ffmpeg combining credits transcode failed: " + err);
            }
          });
      } else {
        callback();
      ...
      if (transcodeInfo.prerollFile) {
        var addPrerollCommandLine = 'ffmpeg -i ' + transcodeInfo.prerollFile + ' -i main.mp4 ' + 
          ...
          '-map "[v]" -map "[a]" -vcodec libx264 -vbr 8000k -s hd1080 -qscale 0 -movflags +faststart ' +
          '-deinterlace out.mp4';
        if (config.veryVerbose) {
          console.log(addPrerollCommandLine);
        }
        child_process.exec(addPrerollCommandLine + ' > output.txt 2>&1', { cwd: workingDir },
          function (err) {
            try {
              assert(!err);
