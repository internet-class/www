...
var doTranscode = function(videoData, config) {
  ...
  var encodeCommandLine = 'ffmpeg -f concat -safe 0 -i concat -i title.mov -filter_complex ' +
    ...
    'fontsize=40 : x=w-tw-20 : y=20" -vcodec libx264 -s hd1080 -qscale 0 -movflags +faststart -deinterlace -c:a aac -ac 1 -ab 128k content.mp4';
  try {
  ...
  if (transcodeInfo.creditsFile) {
    ...
    var encodeCreditsCommandLine = 'ffmpeg -i ' + transcodeInfo.creditsFile + ' -i credits.png ' +
      ...
      '-t ' + transcodeInfo.creditsLength + ' -vcodec libx264 -qscale 0 -movflags +faststart -s hd1080 -deinterlace -c:a aac -ac 1 -ab 128k credits.mp4';
    try {
  ...
  if (transcodeInfo.prerollFile) {
    var addPrerollCommandLine = 'ffmpeg -i ' + transcodeInfo.prerollFile + ' -i main.mp4 ' + 
      ...
      '-map "[v]" -map "[a]" -vcodec libx264 -qscale 0 -movflags +faststart -s hd1080 ' +
      '-deinterlace out.mp4';
    try {
