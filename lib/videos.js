...
var getUploadInfo = function(videoData, config) {
  ...
  try { 
    ...
    if (upload.addCredits) {
      ...
      upload.producerList = common.andList(_.map(upload.producers, function (author) {
        return author.name + "(" + author.credits + ")";
      }));
      assert(upload.producerList);
    }
    if (videoData.created) {
      upload.recordingDate = moment_timezone(new Date(videoData.created)) || moment_timezone();
      assert(upload.recordingData.toISOString() != 'Invalid date');
    } else {
...
Talking: {{ authors }}
Producing: {{ producerList }}

internet-class.org is a blue Systems Research Group (blue.cse.buffalo.edu) production. Visit www.internet-class.org to learn more.`);
...
var doUpload = function(videoData, youtubeClient, config, callback) {
  ...
  var uploadRequest = youtubeClient.videos.insert({
    ...
    autoLevels: uploadInfo.autoLevels,
    stabilize: uploadInfo.stabilize,
    media: {
