...
var doTranscode = function(videoData, config, finished) {
  ...
  var mustHave = videoData.find.fullFiles;
      ...
      child_process.exec(encodeCommandLine + ' > output.txt 2>&1', { cwd: workingDir },
        function (err) {
          try {
            ...
            callback(new Error("ffmpeg transcode failed: " + err));
          }
        });
    }, function (callback) {
      if (transcodeInfo.creditsFile) {
        ...
        child_process.exec(finalCombineCommandLine + ' > output.txt 2>&1', { cwd: workingDir },
          function (err) {
            try {
              ...
              callback(new Error("ffmpeg adding preroll failed: " + err));
            }
          });
      }
    }], function (err) {
        ...
        _.chain(common.walkSync(metalsmith.source()))
          ...
          .each(function(filename) {
            ...
            var newVideoData = {
              ...
              created: moment_timezone.tz(fs.statSync(filename).ctime, config.timezone).toString(),
              transcode: false,
              find: {
