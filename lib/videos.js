...
var getUploadInfo = function(videoData, config) {
  ...
  try { 
    ...
    upload.recordingDate = moment_timezone(videoData.created) || moment_timezone();
...
var upload = function(config) {
  ...
  return function(files, metalsmith, done) {
    ...
    _.each(metalsmith.metadata().videos.byDirectory, function (videosData) {
      _.chain(videosData)
        .filter(function (videoData) {
          return videoData.upload !== false && videoData.find.outputPresent && videoData.youtube === undefined;
        })
        .each(function (videoData) {
          tasks.push(videoData);
        })
