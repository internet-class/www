...
var doTranscode = function(videoData, config, finished) {
  ...
  async.series([
    function (callback) {
      ...
      child_process.exec(titleImageCommandLine + '> output.txt 2>&1', { cwd: workingDir },
        function (err, stdout, stderr) {
          try {
            ...
            return finished(new Error("title image creation failed: " + err));
          }
        });
    }, function (callback) {
      ...
      child_process.exec(titleMovieCommandLine + ' > output.txt 2>&1', { cwd: workingDir },
        function (err) {
          try {
            ...
            return finished(new Error("title movie creation failed: " + err));
          }
        });
    }, function (callback) {
      ...
      if (config.veryVerbose) {
        console.log(encodeCommandLine);
      }
      child_process.exec(encodeCommandLine + ' > output.txt 2>&1', { cwd: workingDir },
        function (err) {
          try {
            assert(!err);
            assert(fs.existsSync(path.join(workingDir, 'content.mp4')));
            finalConcat.push('content.mp4');
            callback();
          } catch (err) {
            if (config.veryVerbose) {
              console.log(fs.readFileSync(path.join(workingDir, 'output.txt')).toString());
            }
            return finished(new Error("ffmpeg transcode failed: " + err));
          }
        });
    }, function (callback) {
        ...
        child_process.exec(creditsImageCommandLine + ' > output.txt 2>&1', { cwd: workingDir },
          function (err) {
            try {
              ...
              return finished(new Error("credit large image creation failed: " + err));
            }
          });
      } else {
        callback();
        ...
        child_process.exec(creditsImageCommandLine + ' > output.txt 2>&1', { cwd: workingDir },
          function (err) {
            try {
              ...
              return finished(new Error("credit image creation failed: " + err));
            }
          });
      } else {
        callback();
      ...
      if (transcodeInfo.creditsFile) {
        var encodeCreditsCommandLine = 'ffmpeg -i ' + transcodeInfo.creditsFile + ' -i credits.png ' +
          ...
          '-t ' + transcodeInfo.creditsLength + ' -vcodec libx264 -vbr 8000k -s hd1080 -qscale 0 -movflags +faststart -deinterlace -c:a aac -ac 1 -ab 128k credits.mp4';
        if (config.veryVerbose) {
        ...
        child_process.exec(encodeCreditsCommandLine + ' > output.txt 2>&1', { cwd: workingDir },
          function (err) {
            try {
              ...
              finalConcat.push('credits.mp4');
              callback();
            } catch (err) {
              if (config.veryVerbose) {
                console.log(fs.readFileSync(path.join(workingDir, 'output.txt')).toString());
              }
              return finished(new Error("ffmpeg credits transcode failed: " + err));
            }
          });
      } else {
        callback();
      ...
      if (transcodeInfo.prerollFile) {
        finalConcat.unshift(transcodeInfo.prerollFile);
      }
      callback();
    }, function (callback) {
      if (finalConcat.length == 1) {
        fs.renameSync(finalConcat[0], path.join(workingDir, 'out.mp4'));
        callback();
      } else {
        var inputString = "";
        var filterString = "";
        _.each(finalConcat, function (file, index) {
          inputString += "-i " + file + " ";
          filterString += "[" + index + ":v:0] [" + index + ":a:0] ";
        });
        var finalCombineCommandLine = 'ffmpeg ' + inputString + '-filter_complex "' + filterString +
          ' concat=n=' + finalConcat.length + ':v=1:a=1 [v] [a]" ' +
        ...
        child_process.exec(finalCombineCommandLine + ' > output.txt 2>&1', { cwd: workingDir },
          function (err) {
            try {
              ...
              finished(new Error("ffmpeg adding preroll failed: " + err));
            }
          });
      }
    }], function (err) {
