...
internet-class.org is a blue Systems Research Group (www.bluegroup.systems) production.`);
...
var getTranscodeInfo = function(videoData, config) {
  ...
  try {
    ...
    transcode.creditsFile = (videoData.creditsFile !== undefined) ? videoData.creditsFile : config.creditsFile;
    if (transcode.creditsFile) {
      videoData.creditsFile = transcode.creditsFile;
      transcode.creditsFile = path.join(config.extras, transcode.creditsFile);
      transcode.producers = videoData.producers;
      assert(transcode.producers);
    }
    transcode.prerollFile = (videoData.prerollFile !== undefined) ? videoData.prerollFile : config.prerollFile;
    if (transcode.prerollFile) {
      videoData.prerollFile = transcode.prerollFile;
      transcode.prerollFile = path.join(config.extras, transcode.prerollFile);
    }
    transcode.postrollFile = (videoData.postrollFile !== undefined) ? videoData.postrollFile : config.postrollFile;
    if (transcode.postrollFile) {
      videoData.postrollFile = transcode.postrollFile;
      transcode.postrollFile = path.join(config.extras, transcode.postrollFile);
    }
    transcode.featuring = videoData.featuring;
    transcode.banner = videoData.banner || config.banner;
    assert(transcode.banner);
  } catch (err) {
...
var doTranscode = function(videoData, config, finished) {
  ...
  if (transcodeInfo.postrollFile) {
    mustHave.push(transcodeInfo.postrollFile);
  }
  ...
  async.series([
    function (callback) {
      ...
      if (transcodeInfo.prerollFile) {
      var probeCommandLine = 'ffprobe -v error -select_streams v:0 -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 ' + transcodeInfo.prerollFile;
      var durationSec = child_process.exec(probeCommandLine, { cwd: workingDir },
          function (err, stdout) {
            try {
              assert(!err);
              videoData.prerollLength = Math.floor(parseFloat(stdout));
              callback();
            } catch (err) {
              if (config.veryVerbose) {
                console.log(stdout);
              }
              callback(new Error("computing preroll length failed: " + err));
            }
          });
      } else {
        videoData.prerollLength = 0;
        callback();
      }
    }, function (callback) {
      if (transcodeInfo.creditsFile) {
      var probeCommandLine = 'ffprobe -v error -select_streams v:0 -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 ' + transcodeInfo.creditsFile;
      var durationSec = child_process.exec(probeCommandLine, { cwd: workingDir },
          function (err, stdout) {
            try {
              assert(!err);
              videoData.creditsLength = Math.ceil(parseFloat(stdout));
              callback();
            } catch (err) {
              if (config.veryVerbose) {
                console.log(stdout);
              }
              callback(new Error("computing credits length failed: " + err));
            }
          });
      } else {
        videoData.creditsLength = 0;
        callback();
      }
    }, function (callback) {
      if (transcodeInfo.postrollFile) {
      var probeCommandLine = 'ffprobe -v error -select_streams v:0 -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 ' + transcodeInfo.postrollFile;
      var durationSec = child_process.exec(probeCommandLine, { cwd: workingDir },
          function (err, stdout) {
            try {
              assert(!err);
              videoData.postrollLength = Math.ceil(parseFloat(stdout));
              callback();
            } catch (err) {
              if (config.veryVerbose) {
                console.log(stdout);
              }
              callback(new Error("computing postroll length failed: " + err));
            }
          });
      } else {
        videoData.postrollLength = 0;
        callback();
      }
    }, function (callback) {
...
var getUploadInfo = function(videoData, config) {
  ...
  try {
    ...
    if (upload.addCredits) {
      ...
      upload.featuring = videoData.featuring;
      if (upload.featuring) {
        upload.featuringList = common.andList(_.map(videoData.featuring, function (author) {
          return author.name + " (" + author.credits + ")";
        }));
      }
    }
...
Credits: Talking: {{ authors }}. Producing: {{ producerList }}.{{#if featuringList}} Featuring: {{ featuringList }}.{{/if}}
...
var find = function(config) {
  return function (files, metalsmith, done) {
    ...
    config = common.processConfig(config, findDefaults);
    ...
    async.series([
      ...
      function (callback) {
        ...
        _.each(updateContents, function (unused, dirname) {
          ...
          var cleaned = _.map(videoDataByDirectory[dirname], function (videoData) {
            delete(videoData.find.metalsmith);
            return videoData;
          });
          files[newFilename] = {
            contents: yamljs.stringify(cleaned)
          };
...
var transcodeDefaults = {
  ...
  extras: path.join(__dirname, 'extras'),
  titleLength: 10,
  banner: 'internet-class.org'
}

var transcode = function(config) {
  config = common.processConfig(config, transcodeDefaults);

  return function(files, metalsmith, done) {
    config.extras = path.join(metalsmith.source(), config.extras);
...
var upload = function(config) {
  config = common.processConfig(config, uploadDefaults);
...
var sync = function(config) {
  config = common.processConfig(config, syncDefaults);
    ...
    _.each(push, function (unused, filename) {
      ...
      if (!(fs.existsSync(backupName))) {
        ...
        fs.symlinkSync(path.relative(path.dirname(filename), backupName), filename);
        if (config.verbose) {
