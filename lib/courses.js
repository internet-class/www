...
var loadDefaults = {
  courseSyllabi: 'courses/**/syllabus.*',
  planName: 'plan.json',
  verbose: false
}
...
var load = function(config) {
  config = common.processConfig(config, loadDefaults);
	...
	return function (files, metalsmith, done) {
    var courses = _.pick(files, function (file, filename) {
      return multimatch(filename, config.courseSyllabi).length > 0;
    });
    metalsmith.metadata().courses = _.map(courses, function (file, filename) {
      var planName = path.join(path.dirname(filename), config.planName);
      file.plan = jsonfile.readFileSync(path.join(metalsmith.source(), planName));
      delete(files[file.plan]);
      return file;
    });
    return done();
  }
