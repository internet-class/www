    ...
    _.chain(files)
      ...
      .each(function (file, planname) {
        ...
        _.each(orderedLessons, function (lessonname, index) {
          ...
          if (deadlines) {
            lesson.deadline = _.find(deadlines, function (deadline) {
              return multimatch(lessonname, deadline.path).length > 0;
            });
            if (lesson.deadline) {
              lesson.deadline = lesson.deadline.time;
              if (lesson.deadline != lastDeadline) {
                lesson.newDeadline = true;
                if (lastDeadline !== undefined) {
                  coursesLessons[lessons[orderedLessons[index - 1]].uuid].lastDeadline = true;
                }
              } else if (index == orderedLessons.length - 1) {
                lesson.lastDeadline = true;
              }
              lastDeadline = lesson.deadline;
            }
          }
