  ...
  return function (files, metalsmith, done) {
    ...
    coursesLessons = _.mapObject(coursesLessons, function (file, uuid) {
      ...
      videos = _.map(videos, function (video) {
        if (!('skip' in video)) {
          video.skip = video.prerollLength;
        }
        if (!('end' in video)) {
          video.end = video.creditsLength + video.postrollLength;
        }
        assert(video.skip !== undefined, JSON.stringify(video));
        assert(video.end !== undefined, JSON.stringify(video));
        return _.pick(video, 'created', 'youtube', 'durationSec', 'skip', 'authors', 'end');
      });
