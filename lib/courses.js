...
var load = function(config) {
  config = common.processConfig(config, loadDefaults);
    ...
    _.each(courses, function (file, filename) {
      ...
      /* START HACK HACK */
      var syllabusPath = path.join('courses', file.slug, 'syllabus.adoc');
      files[path.join('courses', file.slug, 'index.adoc')] = 
        lodash.cloneDeep(files[syllabusPath], function(value) {
          if (value instanceof Buffer) {
            return new Buffer(value);
          }
        });
      files[path.join('courses', file.slug, 'index.adoc')].course = {
        path: path.join('courses', file.slug)
      };
      files[syllabusPath].course = {
        path: path.join('courses', file.slug)
      };
      /* END HACK HACK */
    });
