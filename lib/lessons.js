...
function lessons(config) {
  ...
  return function(files, metalsmith, done) {
    ...
    try {
      _.each(JSON.parse(files[config.lessonIDsFilename].contents), function (lesson) {
    ...
    jsonfile.writeFileSync(path.join(metalsmith.source(), config.lessonIDsFilename), lessonsAsList);
