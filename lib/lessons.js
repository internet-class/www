...
function lessons(config) {
  ...
  return function(files, metalsmith, done) {
    ...
    try {
      var previousLessonsIDs = JSON.parse(files[config.lessonIDsFilename].contents);
      delete(files[config.lessonIDsFilename]);
    } catch (err) { };
    ...
    jsonfile.writeFileSync(path.join(metalsmith.source(), config.lessonIDsFilename), currentLessonIDs);
