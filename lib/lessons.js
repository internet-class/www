...
function lessons(config) {
  ...
  return function(files, metalsmith, done) {
    ...
    try {
      _.each(files, function (file, filename) {
        ...
        if (containingLessons.length > 1 && file.is_lesson) {
          ...
          if (!(file.is_lesson)) {
            var lesson = byDirectory[containingLessons[0]];
            file.lesson = lesson;
            if (path.basename(filename) == config.videosFilename) {
              lesson.videos = yamljs.parse(file.contents.toString());
            }
          } else {
            try {
    ...
    var lessonsByUUID = {};
    _.each(currentLessonIDs, function (filename, uuid) {
      lessonsByUUID[uuid] = files[filename];
      lessonsByUUID[uuid].originalFilename = filename;
    });
    metalsmith.metadata().lessonsByUUID = lessonsByUUID;
    return done();
  }
};
