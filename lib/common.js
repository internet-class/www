...
function transcodeCommand(options) {
  ...
  var resizeContent = _.map(options.content, function (file, index) {
    inputs.push(file);
    return "[" + index + ":v:0] setsar=1/1,scale=1920:-1 [c" + index + "]";
  }).join("; ");
  ...
  if (options.title) {
    filters += "[vcontent] [" + inputs.length + ":v:0] overlay=x=20:y=main_h-overlay_h-20," +
      "drawtext=fontfile=" + options.font + ":text='internet-class.org':fontcolor=white:" +
  ...
  var totalLength = 1;
  if (options.preroll) {
    filters += "[" + inputs.length + ":v:0] setsar=1/1,scale=1920:-1 [pv]; ";
    totalConcat = "[pv] [" + inputs.length + ":a:0] " + totalConcat;
    inputs.push(options.preroll);
    totalLength += 1;
  } 
  if (options.postroll) {
    filters += "[" + inputs.length + ":v:0] setsar=1/1,scale=1920:-1 [av]; ";
