...
function transcodeCommand(options) {
  ...
  var resizeContent = _.map(options.content, function (file, index) {
    inputs.push(file);
    return "[" + index + ":v:0] setsar=1/1,scale=-1:1080 [c" + index + "]";
  }).join("; ");
  ...
  var totalLength = 1;
  if (options.preroll) {
    filters += "[" + inputs.length + ":v:0] setsar=1/1,scale=-1:1080 [pv]; ";
    totalConcat = "[pv] [" + inputs.length + ":a:0] " + totalConcat;
    inputs.push(options.preroll);
    totalLength += 1;
  } 
  if (options.postroll) {
    filters += "[" + inputs.length + ":v:0] setsar=1/1,scale=-1:1080 [av]; ";
